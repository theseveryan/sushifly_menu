...
// ПЕРЕМЕННАЯ ДЛЯ БИБЛИОТЕКИ ЗУМА
let pzInstance = null;
...
// --- ЛОГИКА МОДАЛЬНОГО ОКНА (С БИБЛИОТЕКОЙ) ---
...
function openModal(src) {
    ...
    // ЗАПУСК БИБЛИОТЕКИ PANZOOM
    // maxScale: 4 - максимальный зум
    // minScale: 1 - минимальный зум
    // contain: 'outside' - не дает картинке улететь за край экрана
    if (typeof Panzoom !== 'undefined') {
        pzInstance = Panzoom(modalImg, {
            maxScale: 4,
            minScale: 1,
            startScale: 1,
            contain: 'outside', 
            panOnlyWhenZoomed: true // Двигать можно только если увеличил
        });
        // Включаем поддержку колесика мыши (если с компа)
        modalImg.parentElement.addEventListener('wheel', pzInstance.zoomWithWheel);
    }
}
...
function closeModal() {
    ...
    // Уничтожаем зум при закрытии, чтобы сбросить позицию
    if (pzInstance) {
        pzInstance.destroy();
        pzInstance = null;
    }
